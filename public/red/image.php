...
# Modificado por JP para soportar espacios y paréntesis
if(preg_replace('/[^a-z_.\s\(\)\ñ\Ñ\-\/0-9]/i', '', $image) !== $image) {
    header('HTTP/1.1 400 Bad Request');
...
if ($doSharpen)
{
	...
	// Código agregado por JP
	// Imágenes tomadas desde iPhone tienen problemas de rotación
	$exif = exif_read_data($docRoot . $image);
