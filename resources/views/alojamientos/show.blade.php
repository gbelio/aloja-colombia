  ...
  <div class="form-row">
    ...
    <div class="col-xl-5">
      <div class="form-row show-detalle">
        ...
        <div class="col-md-3 text-center">
          ...
          <div class="show-alojador-fecha">Se registró en<br/>{{$propietario->fechaRegistroFormateada()}}</div>
      ...
      <div class="form-row show-detalle">
        <div class="col-md-12">
          <div class="show-titulo">Cuartos</div>
    ...
    <div class="col-xl-4" id="f">
        ...
        <div class="show-fijo">  
            ...
            {!! Form::open(array('url' => 'alojamientos/' . $alojamiento->id . '#f', 'method' => 'GET', 'class' => '', 'onsubmit' => 'return validarDatosDisponibilidad();')) !!}
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                  ...
                  {!! Form::text('fd', app('request')->input('fd'), ['style' => 'background: white', 'class' => 'form-control', 'required', 'readonly', 'onclick' => '$(".calendarInquilinoContenedor").toggle()' ]) !!}
                        ...
                        <tr>
                          <td colspan="7" style="border: none; padding: 2px;">
                            ...
                            <div style="float: right;">
                              <button class="btn btn-light" id="previous" onclick="previousD('calendar-bodyD')" type="button">Anterior</button>
                              <button class="btn btn-light" id="next" onclick="nextD('calendar-bodyD')" type="button">Siguiente</button>
                            </div>
                        </td>
                        </tr>
                        <tr>
              ...
              <div class="col-md-6">
                {!! Form::label('fh', 'Salida', ['class' => 'show-form-label'] ) !!}
                  {!! Form::text('fh', app('request')->input('fh'), ['style' => 'background: white', 'class' => 'form-control', 'required', 'readonly', 'onclick' => '$(".calendarInquilinoContenedor").toggle()' ]) !!}
                    ...
                    <table class="calendar calendarInquilino" id="calendarH">
                        ...
                        <tr>
                          <td colspan="7" style="border: none; padding: 2px;">
                            ...
                            <div style="float: right;">
                              <button class="btn btn-light" id="previous" onclick="previousH('calendar-bodyH')" type="button">Anterior</button>
                              <button class="btn btn-light" id="next" onclick="nextH('calendar-bodyH')" type="button">Siguiente</button>
                            </div>
                        </td>
                        </tr>
                        <tr>
              ...
              <div class="col-md-6">
                {!! Form::label('h', 'Huéspedes', ['class' => 'show-form-label'] ) !!}
                  {!! Form::number('h', app('request')->input('h'), ['step' => '1', 'min' => '0', 'max' => '50', 'class' => 'form-control', 'required', 'onchange' => '$("#botonReservar").attr("disabled", true);' ]) !!}
              </div>

              
              <div class="col-md-6">
                  <br/>
                  <button class="btn show-boton consultar-disponibilidad-boton" type="submit">Consultar disponibilidad</button>
              </div>
            ...
            <div class="show-reserva-id">Código de reserva: {{$reserva_id}}</div>
              ...
              @if ( $disponible )
                ...
                @if ( $errorCod == 'RESERVADO' )
                    ...
                    <div class="show-reserva-mensaje">
                      Tu reserva está pendiente de aprobación por parte de el alojador.  En el transcurso de las siguientes 24 horas te dará respuesta, de lo contrario tu reserva quedará sin efecto. Se te notificará a tu correo electrónico.
                    </div>
                ...
                @else
                    ...
                    <div class="show-presupuesto-linea">$ {{$alojamiento->precioFormateado($precioValor)}} x {{$diasTotales}} noches<span class="show-presupuesto-linea-subtotal">$ {{$alojamiento->precioFormateado($precioTotal)}}</span></div>
                    @if ($descuento != 0)
                    <div class="show-presupuesto-linea">{{ $descuentoDescripcion }}<span class="show-presupuesto-linea-subtotal show-presupuesto-linea-descuento">- $ {{$alojamiento->precioFormateado($descuento)}}</span></div>
                    @endif
                    <div class="show-presupuesto-linea">Tarifa de limpieza<span class="show-presupuesto-linea-subtotal">$ {{$alojamiento->precioFormateado($precioLimpieza)}}</span></div>
                    <div class="show-presupuesto-linea">Tarifa por servicio<span class="show-presupuesto-linea-subtotal">$ {{$alojamiento->precioFormateado($tarifaServicio)}}</span></div>
                    <div class="show-presupuesto-linea show-presupuesto-linea-total">Total<span class="show-presupuesto-linea-subtotal">$ {{$alojamiento->precioFormateado($totalGeneral)}}</span></div>
                    @if ($deposito != 0)
                    <br/>
                    <div class="show-presupuesto-linea show-presupuesto-linea-deposito">Deposito Reembolsable<span class="show-presupuesto-linea-subtotal">$ {{$alojamiento->precioFormateado($deposito)}}</span></div>
                    <div class="show-presupuesto-senia">Este Alojamiento te solicitará un Depósito Reembolsable el cual deberas entregar en efectivo al llegar a tu Alojamiento y te será devuelto en su   totalidad al finalizar tu estadía si no se presentan daños o faltantes dentro de la propiedad.</div>
                    @endif

                    {!! Form::open(array('onsubmit' => "return confirm('Se efectuará la reserva. ¿Desea continuar?')", 'url' => 'alojamientos/' . $alojamiento->id . '/reservar#f', 'method' => 'POST')) !!}

                      {!! Form::hidden('fd', app('request')->input('fd')) !!}
                      {!! Form::hidden('fh', app('request')->input('fh')) !!}
                      {!! Form::hidden('h', app('request')->input('h')) !!}

                      {!! Form::button('Reservar', ['type' => 'submit', 'id' => 'botonReservar', 'class' => ' btn show-boton', 'style' => 'float: left; margin-right: 5px;' ]) !!}
                ...
                @if ( app('request')->input('opcion') == null && app('request')->input('h') != null )
                  ...
                  @if ( $errorCod == 'MINIMO' )
                    ...
                    <div class="alert alert-danger">
                      Esta propiedad requiere reservar {{ $alojamiento->alquiler_minimo }} días como mínimo en temporada alta.
                    </div>

                  @else

                    <div class="alert alert-danger">
                      Lamentablemente no tenemos disponibilidad en estas fechas para esa cantidad de huéspedes.
                    </div>
            ...
            <div class="show-post-form">
                <div class="show-presupuesto-check"><b>Check In</b>: desde las {{date('H:i', strtotime($alojamiento->check_in)) }}hs.</div>
                <div class="show-presupuesto-check"><b>Check Out</b>: desde las {{date('H:i', strtotime($alojamiento->check_out)) }}hs.</div>
            </div>
            ...
            <div class="show-post-form">
                ...
                Los huéspedes recibirán un reembolso total de la reserva (menos la tarifa de servicio) , si cancelan 48 horas antes del Check In.
                @else
                  @if( $alojamiento->politica_cancelacion == 'M' )
                  Los huéspedes recibirán un reembolso total de la reserva (menos la tarifa de servicio) , si cancelan 7 días antes del Check In.
                  @else
                    @if( $alojamiento->politica_cancelacion == 'E' )
                    Los huéspedes recibirán un reembolso del 50% del total de la reserva (menos la tarifa de servicio) , si cancelan 15 días antes del Check In.
                    @else
                ...
                <div class="show-politicas-notas">*Check in: Se contemplan horas y días completas antes de la hora local de llegada del anuncio (indicada en el correo electrónico de confirmación.</div>
            </div>
      </div>
    </div> 
...
@if ( $errorCod != 'RESERVADO' )  
  ...
  function seleccionarFecha(calendario, fechaF) {
    ...
    if ( calendario == "calendar-bodyD" && ( ( anioActualD == anioActualH && mesActualH < mesActualD ) || ( anioActualD > anioActualH ) ) ) {
